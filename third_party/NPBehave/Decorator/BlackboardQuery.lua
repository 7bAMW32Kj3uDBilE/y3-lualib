local ipairs = ipairs
---@class NPBehave.Decorator.BlackboardQuery
---@overload fun(keys: string[], stopsOnChange: NPBehave.Enum.Stops, query: (fun(): boolean), decoratee: NPBehave.Node): NPBehave.Decorator.BlackboardQuery
...
function BlackboardQuery:__tostring()
    ...
    --     keys = keys .. '|' .. key
    -- end
    keys = table.concat(y3.util.valuesOf(self._keys), '|')
    return keys
end
